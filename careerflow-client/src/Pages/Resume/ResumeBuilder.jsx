...
const ResumeBuilder = () => {
    ...
    const filtered = resumes.filter((r) => {
        ...
        if (timeFilter === "Today") matchesTime = uploadDate.toDateString() === now.toDateString();
        if (timeFilter === "Week") {
        ...
        <div className="p-6 max-w-7xl mx-auto min-h-screen bg-base-100 text-base-content">
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-4 space-y-3 max-h-[650px] overflow-y-auto pr-2">
                    ...
                    {filtered.length > 0 ? (
                        filtered.map((r) => (
                            <div key={r._id} className={`group border p-3 rounded-xl flex justify-between items-center transition-all bg-base-200 hover:border-primary shadow-sm ${preview === r.fileUrl ? "border-primary ring-1 ring-primary" : ""}`}>
                                <div onClick={() => setPreview(r.fileUrl)} className="cursor-pointer flex-1 min-w-0">
                                    <p className="font-semibold text-sm truncate pr-2">{r.name}</p>
                                    <p className="text-[10px] uppercase tracking-wider text-primary font-bold">{r.type}</p>
                                </div>

                                <div className="flex gap-1">
                                    <button onClick={() => openEditModal(r)} className="p-2 text-gray-400 hover:text-primary hover:bg-base-100 rounded-lg transition-colors">
                                        <Edit size={16} />
                                    </button>
                                    <button onClick={() => downloadPDF(r.fileUrl, r.name)} className="p-2 text-gray-400 hover:text-green-600 hover:bg-base-100 rounded-lg transition-colors">
                                        <Download size={16} />
                                    </button>
                                    <button onClick={() => handleDelete(r._id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-base-100 rounded-lg transition-colors">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-base-100 p-6 rounded-2xl w-full max-w-md shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold">{editingResume ? "Update Resume" : "Upload Resume"}</h2>
                            <button onClick={closeModal} className="p-1 hover:bg-base-200 rounded-full transition-colors"><X size={20} /></button>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:bg-base-200 transition-colors relative">
                                    <input type="file" accept="application/pdf" onChange={validateFile} className="absolute inset-0 opacity-0 cursor-pointer" />
                                    <Upload className="mx-auto text-gray-300 mb-2" size={32} />
                                    <p className="text-sm text-gray-500">{file ? <span className="text-primary font-medium">{file.name}</span> : "Click or drag to upload PDF"}</p>
                                </div>
                                {editingResume && !file && (
